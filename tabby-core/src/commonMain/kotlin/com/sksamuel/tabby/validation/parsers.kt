...
/**
 * Widens an existing [Parser] to accept null inputs and rejects blank or nulls.
 */
fun <A, E> Parser<String?, A, E>.notNullOrBlank(ifNullOrBlank: () -> E): Parser<String?, A, E> {
   return parser { input ->
      when (input) {
         null -> ifNullOrBlank().invalid()
         else -> if (input.isBlank()) ifNullOrBlank().invalid() else this@notNullOrBlank.parse(input)
      }
   }
}
...
/**
 * Wraps a [Parser] String -> A by enforcing a min length on the input string.
 */
fun <A, E> Parser<String, A, E>.minlen(len: Int, ifError: (String) -> E): Parser<String, A, E> =
   parser { if (it.length < len) ifError(it).invalid() else this@minlen.parse(it) }

/**
 * Widens a [Parser] to accept nulls, replacing them with a default generated by the given function [ifNull].
 */
fun <I, A, E> Parser<I, A, E>.default(ifNull: () -> A): Parser<I?, A, E> {
   return parser { if (it == null) ifNull().valid() else this@default.parse(it) }
}
